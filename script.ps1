...
$configmap = kubectl get configmap livros-api-env 2>&1
if ($configmap -match "not found" -or $configmap -match "Error") {
    Write-Host "Criando ConfigMap..."
    kubectl create configmap livros-api-env --from-env-file=variaveis_ambientes.env
} else {
    Write-Host "ConfigMap já existe."
}

# 4. Verifica se a imagem existe no minikube, builda se não existir
Write-Host "Verificando imagem livros-api no minikube..."
$imagem = minikube image ls 2>&1 | Select-String "livros-api"
if (-not $imagem) {
    Write-Host "Imagem não encontrada. Buildando dentro do minikube (pode demorar)..."
...
$job = Start-Job -ScriptBlock {
    kubectl port-forward svc/livros-api-service 8000:80
}
